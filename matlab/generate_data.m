function generate_data(box_dir, acap_output_dir, category)
    ...
    for i = 1:size(nameFolds, 1)
        ...
        for j = 1:size(part_names, 2)
            ...
            if exist(deformed_mesh, 'file') && ~exist(fullfile(part_output_dir, 'S.txt'), 'file') && ~exist(fullfile(part_output_dir, 'LOGRNEW.txt'), 'file') && ~exist(mat_file,'file')
                disp(id);

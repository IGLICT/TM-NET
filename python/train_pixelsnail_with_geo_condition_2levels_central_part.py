...
def train(args, epoch, loader, model, optimizer, scheduler, device):
    ...
    for i, (ID, geo_zs, top, bottom) in enumerate(loader):
        model.zero_grad()
        ...
        if args.hier == 'top':
            ...
            target = top
            out, _ = model(top, condition=geo_zs)
        ...
        # loss.backward()
        cross_entropy_loss, extra_loss = model.backward(out, target)
        ...
        loader.set_description(
            (
                f'epoch: {epoch + 1}/{args.epoch}; loss: {cross_entropy_loss.item():.5f}; extra_loss: {extra_loss.item():.5f}; '

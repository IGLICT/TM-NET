...
class VQVAEDataset(Dataset):
    ...
    def __init__(self, image_dir, mode, category=None, part_name=None, height=256, width=256):
        ...
        for folder in self.folders:
            no_patch_files = list(
                                set(glob.glob(os.path.join(self.image_dir, folder, '*.png'))) - set(glob.glob(os.path.join(self.image_dir, folder, '*patch*.png')))
                            )
        ...
        # if self.part_name is not None:
            # self.files = [filename for filename in self.files if self.part_name in filename]
    ...
    def __getitem__(self, idx):
        ...
        # save_image(image, os.path.join('.', basename), normalize=True, range=(-1, 1))
        if self.category == 'car':
